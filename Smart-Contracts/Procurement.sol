...
contract tenderFactory {
    ...
    // stores the addresses of the deployed tenders
...
contract Tender {
    ...
    */
    modifier restricted() {
        require(msg.sender == manager, "You are not authorised, only manager can access this.");
        _;
    }
    ...
    function getBidSummary(uint256 index)
        public
        view
        returns (
            address,
    ...
    */
    ...
    function finalizeBid(uint256 index) public {
        require(!complete,  "Tender results have been declared");
        require(msg.sender == manager, "Only managers can finalise bids.");
        winner = bids[index];
        winIndex = index;
